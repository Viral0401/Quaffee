...
const Predict3 = () => {
  ...
  const handleInputChange = (event) => {
    setRawMaterials(event.target.value.split(",").map((item) => item));
    // setRawMaterials(event.target.value);
  };
    ...
    if (rawMaterials.length > 0) {
      ...
      const prompt = `[
                ...
                },
                {
                  ...
                  "ItemType": "Item",
                  "RawMaterial": "Coffee Beans Ice Arabica",
                  "Qty": 13,
                  "Unit": "GM"
                },
                {
            ...
            based on this data and the input of raw materials suggest what ItemName that can be made from the above dataset, just give 4 5 names no other text, dont give suggestions that are not in the dataset 
              ]`;
      fetchData(prompt);
    }
  };
